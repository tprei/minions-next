...
import {
  ...
  createMacOsLimaSandboxLifecycle,
...
const TEMPLATE_NAME = "minions-template-v1";
const TEMPLATE_DISK_GIB = 20;
const RUNTIME = Object.freeze({
  url: "https://github.com/can1357/oh-my-pi/releases/download/v17.0.4/omp-linux-arm64",
  digest: "d051556529090721e8a8fb66034697cd745fb4a92e1d3cbd485569ea683aab10",
  version: "17.0.4",
});
const ISOLATION_MARKER = "MINIONS_LIMA_ISOLATION_OK";
...
const sourceTemplatePath = resolve(
  scriptDirectory,
  "../packages/adapters/assets/lima/macos-vz-arm64.yaml",
);
...
async function main() {
  try {
    ...
    throw new Error("usage: node scripts/run-lima-synthetic.mjs <prepare|run>");
  } catch (error) {
...
async function loadConfiguration() {
  ...
  const limaHome = requiredAbsoluteEnvironmentPath("MINIONS_LIMA_HOME");
  const stateDirectory = requiredAbsoluteEnvironmentPath("MINIONS_LIMA_STATE_DIRECTORY");
  await assertWritableRoot(limaHome, "MINIONS_LIMA_HOME");
  await assertWritableRoot(stateDirectory, "MINIONS_LIMA_STATE_DIRECTORY");
  await assertVirtualizationSupport();
  const limactlPath = await resolveLimactl();
  const template = Object.freeze({
    limactlPath,
    limaHome,
    sourceTemplatePath,
    templateInstanceName: TEMPLATE_NAME,
    templateDiskGiB: TEMPLATE_DISK_GIB,
    runtime: RUNTIME,
  });
  return Object.freeze({ limaHome, stateDirectory, limactlPath, template });
}

async function prepareMode(configuration) {
  const receipt = await prepareLimaTemplate(configuration.template);
  process.stdout.write(`${JSON.stringify(receipt)}\n`);
}

async function runMode(configuration) {
  const templateFingerprint = requiredFingerprint();
  const expectedTemplateFingerprint = Object.freeze({
    policyVersion: 1,
    digest: templateFingerprint,
  });
  const lifecycle = createLifecycle(configuration, expectedTemplateFingerprint);
  const probe = await lifecycle.probe();
  assertProbe(probe, templateFingerprint);
  ...
  try {
    fixture = await createSandboxContractFixture({
      prefix: "minions-lima-synthetic-",
      templateDigest: contentHash(templateFingerprint),
    });
    ...
    await writeFile(
      ...
      'const child = (await import("node:child_" + "process"))["sp" + "awn"]("/usr/bin/systemd-run", ["--user", "--unit=minions-escape", "--", "/usr/local/bin/node", "-e", "setInterval(() => {}, 1_000)"]); child.on("error", () => process.exit(7)); child.on("exit", (code) => process.exit(code === 0 ? 0 : 7));\n',
      "utf8",
    );
    await writeFile(
    ...
    const dockerResult = await freshTrackedLifecycle.execute({
      ...
      executable: "docker",
      arguments: ["build", "--network=none", "--tag", "minions-lima-synthetic:v1", "."],
    ...
    const isolationResult = await freshTrackedLifecycle.execute({
      ...
      workingDirectory: syntheticFixture.workspace,
      environment: Object.freeze({
        MINIONS_LIMA_PROBE_PATHS: JSON.stringify(isolationPaths),
      }),
      timeoutMs: 30_000,
  ...
  if (failures.length > 0) {
    throw new AggregateError(failures, "Lima synthetic failed");
  }
  if (finalReceipt === undefined) throw new Error("Lima synthetic did not produce a receipt");
  process.stdout.write(`${JSON.stringify(finalReceipt)}\n`);
}

function createLifecycle(configuration, expectedTemplateFingerprint) {
  return createMacOsLimaSandboxLifecycle({
    limactlPath: configuration.limactlPath,
    limaHome: configuration.limaHome,
...
function createSyntheticFixture(fixture) {
  ...
  const policy = Object.freeze({
    ...
    resources: Object.freeze({
      ...
      storageMiB: TEMPLATE_DISK_GIB * 1_024,
...
async function isolationProbePaths(configuration, instanceId, fixture) {
  ...
  await assertDirectory(keychains, "host Keychains");
  await assertDirectory(fixture.siblingWorkspace, "fixture sibling workspace");
  const instanceDirectory = join(configuration.limaHome, instanceId);
  const controlSocket = join(instanceDirectory, "ha.sock");
  const runtimeSocket = join(instanceDirectory, "ssh.sock");
  await assertSocket(controlSocket, "Lima hostagent control socket");
  await assertSocket(runtimeSocket, "Lima SSH runtime socket");
  await assertRegularFile(fixture.daemonSocket, "fixture daemon socket sentinel");
  return Object.freeze([
    Object.freeze({ path: hostHome, kind: "directory" }),
    Object.freeze({ path: keychains, kind: "directory" }),
    Object.freeze({ path: controlSocket, kind: "file" }),
    Object.freeze({ path: runtimeSocket, kind: "file" }),
...
function assertContractScenarioCount() {
...
function assertProbe(probe, expectedFingerprint) {
  if (!probe.available) {
    throw new Error(`${probe.message}. Remediation: ${probeRemediation(probe)}`);
  }
  if (probe.templateFingerprint !== expectedFingerprint) {
    throw new Error("Lima capability probe returned the wrong template fingerprint");
  }
  ...
  if (
    ...
    !capabilities.supportedNetworkProfiles.includes("implementation")
  ) {
    throw new Error("Lima capability probe is missing a required sandbox capability");
  }
  assertContractScenarioCount();
}

function probeRemediation(probe) {
  if (probe.failureCode === "capability_unavailable") {
    if (probe.message.includes("Virtualization.framework")) {
      return "Enable hardware virtualization on the execution host.";
    }
    return "Select the Podman backend on Linux/WSL or attach an Apple Silicon Mac host.";
  }
  if (probe.failureCode === "limactl_invalid" || probe.failureCode === "command_failed") {
    return "Install Lima and ensure limactl is executable on PATH.";
  }
  return "Use explicit writable Lima and state roots and rebuild the immutable template.";
}

function assertAppleSiliconHost() {
  if (platform() !== "darwin" || arch() !== "arm64") {
    throw new Error(
      "the macOS Lima backend requires Apple Silicon macOS. Remediation: Select the Podman backend on Linux/WSL or attach an Apple Silicon Mac host.",
    );
  }
}

async function assertVirtualizationSupport() {
  try {
    const result = await executeFile("/usr/sbin/sysctl", ["-n", "kern.hv_support"], {
      env: Object.freeze({ PATH: "/usr/bin:/bin:/usr/sbin:/sbin", LANG: "C", LC_ALL: "C" }),
      timeout: 10_000,
      maxBuffer: 16_384,
    });
    ...
    throw new Error(
      "Apple Virtualization.framework is unavailable. Remediation: Enable hardware virtualization on the execution host.",
      { cause: error },
    );
  }
  throw new Error(
...
function requiredAbsoluteEnvironmentPath(name) {
  ...
  if (
    ...
    throw new Error(
      `${name} must be an absolute normalized path. Remediation: Run host setup with explicit dedicated Lima and state paths.`,
    );
  }
  return value;
}
...
async function assertWritableRoot(path, name) {
  try {
    ...
    throw new Error(
      `${name} must be an explicit writable real directory. Remediation: Run host setup with explicit dedicated Lima and state paths.`,
      { cause: error },
    );
  }
}

async function resolveLimactl() {
  const pathValue = process.env.PATH;
  if (typeof pathValue !== "string" || pathValue.length === 0) {
    throw new Error(
      "limactl is unavailable. Remediation: Install Lima and ensure limactl is executable on PATH.",
    );
  }
  for (const directory of pathValue.split(delimiter)) {
    const candidate = join(directory.length === 0 ? process.cwd() : resolve(directory), "limactl");
    try {
...
async function assertSocket(path, field) {
  const metadata = await lstat(path);
  if (!metadata.isSocket()) throw new Error(`${field} is unavailable`);
}
...
function requiredFingerprint() {
  const value = process.env.MINIONS_LIMA_TEMPLATE_FINGERPRINT;
  if (typeof value !== "string" || !/^[0-9a-f]{64}$/u.test(value)) {
    throw new Error("MINIONS_LIMA_TEMPLATE_FINGERPRINT must be an exact lowercase 64-hex digest");
  }
  return value;
}
